...
import { MembershipPeriod, SerializedMembershipPeriod } from "../types/membership.types";
...
export async function getAll(): Promise<MembershipPeriod[]> {
  ...
  const serializedPeriods: SerializedMembershipPeriod[] = JSON.parse(data);

  const memberships: MembershipPeriod[] = serializedPeriods.map((mp) => ({
    ...mp,
    start: new Date(mp.start),
    end: new Date(mp.end),
  }));
